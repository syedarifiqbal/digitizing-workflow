    ...
    <AppLayout>
        <template #header>
            <div class="flex flex-col gap-1">
                <h2 class="text-lg font-semibold text-gray-900">Webhook Logs</h2>
        ...
        <div class="py-8">
            <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white shadow-sm rounded-lg border border-gray-200">
                    <div class="px-5 py-4 border-b border-gray-100 flex flex-wrap items-center gap-4">
                        <select
                            v-model="eventFilter"
                    ...
                    <div class="divide-y divide-gray-100">
                        ...
                        >
                            <button
                                ...
                                class="w-full px-5 py-3 text-left hover:bg-gray-50 transition flex items-center gap-4"
                            >
                                <span
                                    class="inline-flex h-2 w-2 rounded-full flex-shrink-0"
                                    :class="log.success ? 'bg-green-500' : 'bg-red-500'"
                                ></span>
                                <span class="text-sm font-mono text-gray-900 w-40 flex-shrink-0">{{ log.event }}</span>
                                <span class="text-xs text-gray-500 truncate flex-1">{{ log.url }}</span>
                                ...
                                <span class="text-xs text-gray-400 flex-shrink-0 w-36 text-right">{{ log.created_at }}</span>
                            </button>

                            <div
                                v-if="expandedLog === log.id"
                                class="px-5 py-4 bg-gray-50 border-t border-gray-100 space-y-3"
                            >
                                <div>
                                    <p class="text-xs font-semibold text-gray-500 uppercase">Payload</p>
                                    <pre class="mt-1 text-xs text-gray-700 bg-white border border-gray-200 rounded p-3 overflow-x-auto max-h-48">{{ formatPayload(log.payload) }}</pre>
                                </div>
                                <div v-if="log.response_body">
                                    <p class="text-xs font-semibold text-gray-500 uppercase">Response</p>
                                    <pre class="mt-1 text-xs text-gray-700 bg-white border border-gray-200 rounded p-3 overflow-x-auto max-h-48">{{ log.response_body }}</pre>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Pagination -->
                    <div v-if="logs.last_page > 1" class="px-5 py-3 border-t border-gray-100 flex items-center justify-between text-sm">
